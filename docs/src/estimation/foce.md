...
3. **The reconvergence is tighter than the fit.** Because the Hessian differences the reconverged gradient, an EBE solved only to the fit tolerance (`inner_tol`, default `1e-4`) leaves a residual the difference quotient amplifies by `1/(2h)` — a noise floor that dominates the truncation error and scatters the SEs. ferx reconverges the covariance-step EBEs to at most `1e-6` (and never looser than the user's `inner_tol`), matching the tighter conditioning NONMEM uses in `$COVARIANCE` (issue [#223](https://github.com/FeRx-NLME/ferx-core/issues/223)). This sharpens the SEs without a new option; it costs a few extra warm-started inner iterations per perturbed point.
