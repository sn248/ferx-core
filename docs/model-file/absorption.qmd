...
inverse-Gaussian** model (`igd`), and the **Weibull** model (`weibull`); more
input-rate models (the zero-order families) are planned — see
...
`transit(...)`, `igd(...)`, and `weibull(...)` are currently served by numerical
ODE integration, so they can only feed an ODE disposition. Combining one with an
...
`plans/absorption-models.md`), so this restriction is interim for them. **Weibull
has no elementary closed form, so its error rule is permanent** — `weibull(...)`
will always require an explicit ODE disposition.
