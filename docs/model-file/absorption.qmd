...
by the same mechanism on the event-driven walk.

`zero_order(dur)` reverts to **finite differences** only when combined with an
estimated lagtime, an EVID 3/4 reset, or time-varying covariates (those route to the
FD fallback). See [#530](https://github.com/FeRx-NLME/ferx-core/issues/530).
:::
