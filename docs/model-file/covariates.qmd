...
subject-static expression `obs_scale` divisors since #590). Two combinations still
fall back to the finite-difference gradient: time-varying covariates with **dose
lagtime**, and non-IOV ODE time-varying covariates with **expression-based output
scaling** (`obs_scale = <expr>` that references parameters or covariates). (See
[FOCE/FOCEI](../estimation/foce.qmd) for the optimizer/gradient interaction.)
