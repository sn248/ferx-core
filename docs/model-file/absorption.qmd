...
`parallel` (only smooth `first_order` terms) keeps **exact analytic** (`Dual2`)
FOCE/FOCEI gradients — including the derivative with respect to the pathway fraction.
`mixed` contains a `zero_order` term, so — exactly as for a plain `zero_order` model —
the zero-order **duration and fraction** are differentiated by **finite differences**
(the moving-boundary case, [#530](https://github.com/FeRx-NLME/ferx-core/issues/530));
the first-order pathway's parameters stay analytic.
