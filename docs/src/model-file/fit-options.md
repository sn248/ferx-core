...
| `covariance_method` | `r`, `s`, `rsr` | `r` | Which covariance estimator to assemble, mirroring NONMEM `$COVARIANCE MATRIX=`. `r` = inverse Hessian `R⁻¹` (model-based; the default). `s` = inverse score cross-product `S⁻¹` (empirical information). `rsr` = the Huber–White sandwich `R⁻¹SR⁻¹` (robust to model mis-specification; NONMEM's default). `s`/`rsr` are currently supported for FOCEI and IOV fits (they need the per-subject score, whose Ω-prior term is only consistent under interaction). Requires `covariance = true`. |
