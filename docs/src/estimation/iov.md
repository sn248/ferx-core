...
- **Cross-occasion dose carryover (Option A).** Each occasion's predictions are computed with that occasion's PK parameters applied to the *entire* dose history, then only that occasion's observation rows are scored. So a dose from an earlier occasion contributes to a later occasion's observations using the *later* occasion's CL/V/etc., not the dose's own occasion. For sparse designs with non-overlapping occasion windows the difference is negligible, but for designs with substantial carryover (e.g. warfarin, where occasion 2 doses before occasion 1 has washed out) the IOV FOCE OFV can sit tens of units below a NONMEM reference at matched parameters — see `tests/warfarin_iov_nonmem.rs`. Full per-dose occasion accounting is future work.
