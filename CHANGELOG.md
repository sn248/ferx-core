...
- **Built-in absorption input-rate forcing** (`igd`/`transit`/`weibull`/`first_order`)
  combined with **EVID 3/4 resets** or **time-varying covariates** now gets exact
  analytic FOCE/FOCEI sensitivities instead of finite differences (#486): the
  reset case threads the already-tracked `reset_floor` into the shared forcing
  helper (turning off a dose's pre-reset tail, matching the infusion rule), and
  the TV-cov case wires the same helper into the event-driven walk, hoisting the
  forcing's dose-invariant constants fresh per segment as the PK snapshot
  changes. Combined with an **estimated lagtime**, `igd`/`transit`/`first_order`
  are also now analytic: the continuous `∂R_in/∂lag` flows through the walk's
  dual time-after-dose, and the forcing's onset at the dose's lagged arrival is
