...
`obs_scale` like `1000 / V`, not just the outer gradient. (Per-CMT `obs_scale[CMT=N]`
falls outside the analytic provider's scope entirely, and the IOV inner loop and the
ODE path keep their existing FD routes for expression scaling.) Result-neutral —
estimates and SEs are unchanged.
