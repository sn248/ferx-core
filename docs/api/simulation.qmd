...
2. Generate predictions through the same time-varying-covariate-aware dispatcher
   used by estimation and `predict()` (`compute_predictions_with_tv`). PK
   parameters are recomputed per event from each row's covariate snapshot, so a
   covariate that changes over a subject's record (e.g. body weight) drives the
   prediction at each time — not just the baseline value. Subjects with no
   time-varying covariates stay on the single-evaluation fast path
   (`pk_param_fn(theta, eta, covariates)`), so their output is unchanged.
...
The same per-event covariate snapshots drive the NPDE/NPD diagnostics and the
NCA-based start-value sweep, so all simulation-backed surfaces agree.
