...
| `imp_defensive_alpha` | `0.1` | Defensive-mixture weight (issue #528). Each subject draws this fraction of its samples from the prior `N(0, Ω)` rather than the mode-centred proposal, and every sample is scored under the mixture density. This bounds the importance weights so a weakly-identified subject — e.g. an analytical `[initial_conditions]` baseline whose `V` cancels in the amplitude — can't hijack the weighted M-step and walk θ to the bounds. Must be in `[0, 1)`; set `0` for the legacy single-proposal sampler. Applies to `imp` and `impmap`. |
