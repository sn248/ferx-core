...
- **Importance Sampling (IMP), evaluation-only** (`method = [..., imp]` with `imp_eval_only = true`) — a Monte-Carlo marginal likelihood estimate (`−2 log L`) more accurate than the Laplace approximation, for model comparison when the FOCE OFV may be biased (sparse data, complex random-effect structure). This is the post-estimation *scoring* use of [IMP](importance-sampling.md); without `imp_eval_only` the same `imp` token *estimates* instead (see Statistical methods above).
