...
snapshot, all since #486). Time-varying covariates with **dose lagtime** on the
closed-form analytical models still fall back to the finite-difference gradient, as
does an ODE model that combines `init(...)` with an **EVID 3/4 reset** (production
re-seeds `init` at the reset-event snapshot). (See
