...
serves an η-dependent `obs_scale` analytically — on both loops, on its static walk —
since #486. (Per-CMT `obs_scale[CMT=N]` falls outside the analytic provider's scope
entirely, and LTBS, time-varying covariates, and the IOV inner loop keep their FD
routes for expression scaling.) Result-neutral — estimates and SEs are unchanged.
...
`y = state/V` was the only analytic route). The same subject-static quotient
rule the closed-form provider uses (`apply_expression_scale_*`) is applied to
the ODE prediction jet for both the outer θ/Ω/σ gradient and the inner EBE
η-gradient. It is served on the **static** dual walk only: combined with **LTBS**
or **time-varying covariates** an ODE expression `obs_scale` still falls back to
FD (the walk applies LTBS before the η/θ chain, and a TV-cov scale would be
per-event rather than subject-static).
