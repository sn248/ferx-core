...
| `fd_hessian_step` | float | `1e-2` | Relative step size for the finite-difference Hessian used in the covariance step. The actual perturbation for parameter `i` is `fd_hessian_step * (1 + |x_hat[i]|)`. Increase (e.g. `0.1`) when the default produces `"Hessian has ill-conditioned entries"` warnings; decrease (e.g. `1e-3`) on smooth OFV surfaces where FD noise is the primary concern. Must be positive and finite. |
