...
**`lbfgs` / `nlopt_lbfgs`** — strong choices on **analytical 1-/2-/3-cpt FOCE/FOCEI** fits, where they get an exact bias-free gradient (see [Analytic FOCE / FOCEI gradient](#analytic-foce--focei-gradient-analytical-pk-models)) that reaches the true optimum at wall-time comparable to `bobyqa` (and several × faster than the now-deprecated built-in BFGS). Since #410 they also get the exact gradient on **in-scope user-`[odes]` models** (bolus/infusion dosing, `ObsCmt`/Form-C/per-CMT readout, `F`, resets, static covariates, and — since #439 — time-varying covariates on bolus dosing), so `lbfgs` is a good choice there too; out-of-scope ODE features (steady state, IOV, `input_rate`/SDE, and TV-covariates combined with infusion/SS/reset) still fall back to the finite-difference gradient and its fixed-EBE bias — prefer `bobyqa` for those. **`mma`** is rarely needed.
