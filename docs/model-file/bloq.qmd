...
The M3 censored likelihood is differentiated with **exact analytic sensitivities** for FOCEI and non-interaction FOCE — including with inter-occasion variability (IOV) — on both the closed-form (1/2/3-compartment) and user-`[odes]` paths. The censored data term $-\log\Phi$ and its derivatives ride the same per-subject gradient assembly as quantified observations; the random-effect Hessian uses them while the Laplace $\log|\tilde H|$ excludes censored rows. The only M3 combination that still falls back to finite differences is the ODE triple **M3 + IOV + `iiv_on_ruv`** (residual-error IIV on an `[odes]` model).
