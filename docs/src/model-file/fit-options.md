...
| `multi_start` | integer | `1` | Run this many independent fits — the first from the user's initial values, subsequent ones from log-uniform random perturbations within the declared bounds — and return the one with the lowest OFV. Useful for poorly-identified models where the optimiser can land in different local minima depending on the starting point. The returned `FitResult.warnings` includes a `multi-start:` summary of the OFV spread across starts; a wide spread (≫1 OFV unit) is a strong signal that the model has multiple local optima. Default `1` (no multi-start). |
| `multi_start_seed` | integer | — | RNG seed for the initial-value perturbations. Omit for non-reproducible runs (different draws each time); set for reproducibility. |
