...
A few SS sub-cases still route to the finite-difference fallback (correct, just slower):

- **SS + estimated lagtime** — the pre-arrival window `[t_dose, t_dose+lag]` must read the
  previous interval's steady-state tail, not yet seeded by the dual walk.
- **SS + a rate-defined infusion under `F ≠ 1`** — the equilibration window would need to
  scale with `F` (a moving boundary not yet carried per cycle).
