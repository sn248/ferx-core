...
`zero_order(dur)` is, by definition, a zero-order infusion of duration `dur`, so
it is anchored **in-engine** against an explicit infusion rather than a separate
NONMEM run: a `zero_order(dur=DUR)` model fed by a bolus must predict identically
to the *same disposition* fed by an explicit infusion of duration `DUR` (rate
`Dose/DUR`). `tests/zero_order_absorption.rs` asserts this equivalence across the
whole curve — the in-window ramp, the window-end kink, and the post-window decay —
to `1e-6`, which pins the value path exactly (the constant-rate input reuses the
same modeled-duration forcing as NONMEM's `RATE=−2`/`D1`). The forcing is
additionally validated by the absorption-independent **mass-balance invariant**
`∫A dt = F·Dose/ke` and by an end-to-end **duration-recovery fit** (gated behind
`--features slow-tests`). `dur`'s sensitivity is finite-difference (see the note
above), so this model uses the FD gradient path rather than the analytic `Dual2`
route.
