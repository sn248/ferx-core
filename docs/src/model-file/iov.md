...
- **Inner-loop gradients outside the analytical scope use finite differences** for IOV models. Within the analytical 1-/2-/3-compartment scope the inner Jacobian and the outer gradient now come from the exact analytic sensitivity provider (see [Inner loop](#inner-loop) and [Outer-loop gradient](#outer-loop-gradient-ebe-re-convergence-for-iov)); ODE and steady-state IOV models still fall back to finite differences.
