...
the work stays visible to everyone (see [Communication](sdlc.qmd#3-communication)).
...
priority (see [Gold-standard validation](sdlc.qmd#7-gold-standard-validation)),
...
workflow](sdlc.qmd#4-the-development-workflow-with-ai)): bug reports, parity
...
- **Feature:** state **Why, How, and the design choices**, and who benefits.
  ...
  bar](sdlc.qmd#1-philosophy-and-quality-bar).
...
[SDLC](sdlc.qmd#4-the-development-workflow-with-ai); the short version for a
contributor:
...
2. **Branch off the latest `main`** (trunk-based — see [Git
   workflow](sdlc.qmd#5-git-workflow)); keep it focused on one issue.
3. **Test it.** Add a test at the right tier (see the [testing
   pyramid](sdlc.qmd#8-cicd-and-quality-gates)); for anything numerical add a
   NONMEM comparison ([Gold-standard
   validation](sdlc.qmd#7-gold-standard-validation)). Every feature needs a
   test; every bug fix needs a regression test that fails without it.
4. **Keep both sides in sync.** ferx-core ↔ ferx-r ([Cross-repo
   synchronization](sdlc.qmd#10-cross-repo-synchronization)) and docs updated;
   on the R side, regenerate `man/` and keep `R/*.R` ASCII-only (see [Pull
   requests](sdlc.qmd#6-pull-requests)).
...
7. **Finish the [Definition-of-done checklist](sdlc.qmd#12-definition-of-done)**
