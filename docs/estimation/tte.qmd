...
|-----------|------|-----------|---------------|--------------------|
| TVLAMBDA (rate) | 0.10 | 0.0768 | 0.0770 | _run `nonmem_frailty.ctl`_ |
...
> NONMEM reports `THETA(1)=log λ`; compare `exp(THETA(1))` to ferx `TVLAMBDA`. The hand-off
> `nonmem.ctl` was run as **fixed-effects** (`$OMEGA 0 FIX`, FO) and matches ferx/`survreg`
> exactly (λ 0.0745, −2LL 589.888); filling this **frailty** column needs the corrected
> `nonmem_frailty.ctl` (estimated ω² + `LAPLACIAN` — FO is invalid for a likelihood model with
> random effects). **Gompertz** (intended fixed-effects) is already fully cross-validated: NONMEM
> matches ferx to OFV 3011.12. A small OFV offset from differing normalising constants is
> expected for nlmixr2 (§11.3).
