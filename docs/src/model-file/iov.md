...
Optimization uses BFGS. Within the analytical 1-/2-/3-compartment scope the inner-loop H-matrix (the Jacobian ∂f/∂[η, κ] over the stacked random effects) is now taken from the exact analytic second-order-dual sensitivity provider rather than finite differences; models outside that scope (ODE models, steady-state IOV) keep the finite-difference Jacobian.
...
Outside that scope — ODE models, **steady-state IOV doses**, or when the analytic component is non-finite — the outer gradient falls back to **central finite differences that re-converge the inner loop at each perturbed point** (`reconverged_fd_gradient`), which restores the correct descent direction for the variance components at a cost of `2·n_free` inner-loop solves per gradient. The default **BOBYQA** optimizer is derivative-free and uses neither path. Setting `reconverge_gradient_interval = 1` forces the reconverged-FD gradient even within the analytical scope, as a documented escape hatch.
...
- **Inner-loop gradients outside the analytical scope use finite differences** (not AD) for IOV models. Within the analytical 1-/2-/3-compartment scope the inner Jacobian and the outer gradient now come from the exact analytic sensitivity provider (see [Inner loop](#inner-loop) and [Outer-loop gradient](#outer-loop-gradient-ebe-re-convergence-for-iov)); ODE and steady-state IOV models still fall back to finite differences.
