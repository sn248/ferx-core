...
zero-order `mixed`/`parallel` family is still planned — see `plans/absorption-models.md`.
...
- The same multiplier works for `transit`/`weibull` terms. A fraction on
  `zero_order(...)` (the `mixed`/`parallel` zero-order family) is **not yet
  supported** ([#505](https://github.com/FeRx-NLME/ferx-core/issues/505)).
