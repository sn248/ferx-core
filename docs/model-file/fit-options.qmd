...
| `optimizer` | `bobyqa`, `slsqp`, `nlopt_lbfgs`, `mma`, `trust_region` (plus deprecated aliases `bfgs`/`lbfgs` → `nlopt_lbfgs`) | `bobyqa` | Outer-loop optimisation algorithm. Applies to `method = foce` / `focei` and to the FOCEI polish phase of `method = gn_hybrid`. Ignored (with a runtime warning) under `method = saem` / `imp` / `gn` — see the [Outer Optimizers](../estimation/optimizers.qmd) page for the applicability matrix and the rationale for the BOBYQA default. `bfgs` and `lbfgs` are deprecated aliases that now select `nlopt_lbfgs` (the hand-rolled built-in BFGS/L-BFGS is slated for removal — see #483). |
...
| `nlopt_lbfgs` | NLopt L-BFGS | Analytic-gradient FOCE/FOCEI models; large parameter spaces |
